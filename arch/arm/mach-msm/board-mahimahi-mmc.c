...
//	else
//		return mahimahi_sd_status;
}

/*#define MAHIMAHI_MMC_VDD	(MMC_VDD_28_29 | MMC_VDD_29_30)*/
...
static int
mahimahi_sdslot_register_cb(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (sdslot_status_cb)
		return -EAGAIN;
...
static unsigned int mahimahi_sdslot_type = MMC_TYPE_SD;
...
static struct mmc_platform_data mahimahi_sdslot_data = {
	...
	.register_status_notify = mahimahi_sdslot_register_cb,
	.slot_type		= &mahimahi_sdslot_type,
};
...
static int
mahimahi_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int mahimahi_wifi_cd;	/* WiFi virtual 'card detect' status */
...
EXPORT_SYMBOL(mahimahi_wifi_set_carddetect);
...
EXPORT_SYMBOL(mahimahi_wifi_power);
...
int __init mahimahi_init_mmc(unsigned int sys_rev)
{
	...
	gpio_set_value(MAHIMAHI_GPIO_WIFI_SHUTDOWN_N, 0);
